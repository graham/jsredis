...
return new_list;};var obs={};var to_remove=[];var obs_id=1;var next_id=function(){obs_id+=1;return obs_id;};var smart_add=function(name,o){if(obs[name]==undefined){obs[name]=[o];}else{obs[name].push(o);}};var event_on=function(name,cb){var uid=next_id();smart_add(name,[cb,true,uid]);return uid;};var event_once=function(name,cb){var uid=next_id();smart_add(name,[cb,false,uid]);return uid;};var event_fire=function(name){if(obs[name]!=undefined){var ll=obs[name];var args=[obs[2]].concat(arguments);obs[name]=publish_event_to_list(ll,args);}
if(obs['*']!=undefined){var ll=obs['*'];var args=[obs[2]].concat(arguments);obs['*']=publish_event_to_list(ll,args);}};var publish_event_to_list=function(ll,args){var new_list=[];var now_final=false;for(var i=0;i<ll.length;i+=1){if(x_in_list(ll[i][2],to_remove)){to_remove=remove_x_from_list(ll[i][2],to_remove);}else{now_final=ll[i][0].apply(null,args);if(now_final!=false){if(ll[i][1]){new_list.push(ll[i]);}}}}
return new_list;};var reset=function(){obs={};};var remove=function(uid){to_remove.push(uid);};return{'on':event_on,'once':event_once,'fire':event_fire,'reset':reset,'remove':remove};})();var storage=(function(){var StorageInstance=function(){var si=this;this.db=null;db_hook=indexedDB.open("jsredis_storage");db_hook.onupgradeneeded=function(){var db=db_hook.result;si.on_load(db);var store=db.createObjectStore("keys",{keyPath:"key"});var keyIndex=store.createIndex("by_key","key",{unique:true});var meta_store=db.createObjectStore("meta_keys",{keyPath:"key"});var metKeyIndex=meta_store.createIndex("by_key","key",{unique:true});};db_hook.onsuccess=function(){si.on_load(db_hook.result);};};StorageInstance.prototype.on_load=function(db){this.db=db;console.log("database opened.");};StorageInstance.prototype.transaction=function(table){return this.db.transaction(table,"readwrite");};StorageInstance.prototype.table=function(name){return new Table(this,name);};var Table=function(si,table_name){this.si=si;this.table_name=table_name;};Table.prototype.search_subindex=function(key,cb){var tx=this.si.transaction(this.table_name);var store=tx.objectStore("keys");var index=store.index("by_key");var request=index.openCursor(IDBKeyRange.bound(key+"/",key+":"));var results=[];request.onsuccess=function(){var cursor=request.result;if(cursor){results.push(cursor.value);cursor.continue();}else{if(cb){cb(key,results);}else{console.log(key,results);}}}};Table.prototype.set=function(key,value,cb){var tx=this.si.transaction(this.table_name);var store=tx.objectStore("keys");var index=store.index("by_key");var request=index.openCursor(IDBKeyRange.only(key));request.onsuccess=function(){var cursor=request.result;var new_value={'key':key,'value':value,'type':value.constructor.name};if(cursor){cursor.update(new_value);}else{store.put(new_value)
tx.oncomplete=function(){if(cb){cb(key,new_value);}};}}};var handle_result=function(table,request,key,callback){return(function(){var matching=request.result;if(matching!==undefined){if(callback){callback.apply(table,[key,matching]);}else{console.log("Console: "+key+" == "+matching.value+" ("+matching.type+").");}}else{if(callback){callback.apply(table,[key,undefined]);}else{console.log("Console: "+key+" == "+null+" ("+null+").");}}})};Table.prototype.get=function(key,cb){var tx=this.si.transaction(this.table_name);var store=tx.objectStore("keys");var index=store.index("by_key");var request=index.get(key);request.onsuccess=handle_result(this,request,key,cb);};Table.prototype.lpush=function(key,new_value,cb){var tx=this.si.transaction(this.table_name);var store=tx.objectStore("keys");var index=store.index("by_key");var request=index.get(key);this.get(key,function(key,value){if(value!==undefined){this.set(key,[new_value].concat(value.value),cb);}else{this.set(key,[new_value],cb);}});};Table.prototype.rpush=function(key,new_value,cb){var tx=this.si.transaction(this.table_name);var store=tx.objectStore("keys");var index=store.index("by_key");var request=index.get(key);this.get(key,function(key,value){if(value!==undefined){this.set(key,value.value.concat([new_value]),cb);}else{this.set(key,[new_value],cb);}});};return{"Storage":StorageInstance};})();if(typeof jsredis_module!=='undefined'){jsredis_module.exports.storage=storage;}
